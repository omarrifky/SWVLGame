...
public class Man3Controller : MonoBehaviour
{
    Animator anim;
   ...
   public AudioSource latebuzzer;
    ...
    void Update()
    {
        ...
        if (station1.transform.position.z <= Bus.transform.position.z + 2 && station1.transform.position.z + 5 >= Bus.transform.position.z && Bus.transform.position.x >= 3.6 && Time.time < 57)
        {
            late = false;
        }
        ...
        if(busarrived ==true && destinationlate == false)
        {
            ...
            anim.SetBool("StartDancing", busarrived);
             transform.position = new Vector3(6.66f, 0.42f, 669.41f);
        }
